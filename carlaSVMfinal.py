...
def predict_steering(img):
    ...
    if not hasattr(predict_steering, "_model"):
        model_path = "svm_lane_modeljob.joblib"
    ...
    if model is not None:
        try:
            ...
            arr = arr.reshape((img.height, img.width, 4))  # (H, W, 4)
            bgr = arr[:, :, :3][:, :, ::-1]                # BGRA → BGR
            gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
            gray = cv2.resize(gray, (128, 128))           # IMAGE_SIZE=(128,128)
            ...
            feat = hog(edges,
                       ...
                       block_norm='L2-Hys',
                       visualize=False)
            feat = feat.reshape(1, -1)                   # 变成 (1, D)，SVM 要求 2D 输入
            raw_pred = model.predict(feat)[0]            
            #pred = float(raw_pred)                      #太极端(too extreme)
            mapping = {-1: -0.5, 0: 0.0, 1: +0.5}
            pred = mapping.get(int(raw_pred), 0.0)
